...
Формула Бернуллі або біноміальний розподіл, описує схему коли одна подія повторюється багато разів з однаковою ймовірністю.
Спочатку розраховується комбінація $C_n^k$, скількома способами можна вибрати $k$ успішних випробувань серед $n$.
Далі множник $p^k$ дає ймовірність того, що саме ці $k$ вибраних випробовувань будуть успішними. Множник $(1-p)^{n-k}$ дає ймовірність того, що решка $n-k$ випробувань будуть неуспішними.
Добуток цих частин дає ймовірність однієї конкретної конфігурації успіхів і невдач.
Оскільки таких конфігурацій $C_n^k$, ми множимо на їх кількість. Результат - повна ймовірність того, що успіх відбудеться рівно $k$ разів.
...
```cpp
auto single = [&](long long i)
{
    double comb = Probability::combination_by_product(n, i);
...
pandoc README.md -s \
  ...
  --metadata subtitle="Тема 4. Біноміальний розподіл" \
