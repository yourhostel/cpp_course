...
Нехай об’єктів (12 телевізорів), розбитих на $k$ типів(`2 типи в задачі{5 електр.комп.; 7 корпус}`)
...
або  $$N = \sum_{i=1}^{k}N_i$$

Нехай серед вибраних $m$ об’єктів виявилося $x_1, x_2, ... , x_k$ об’єктів відповідно $1, 2, ... , k$-го типу. Тоді
$0\leq x_i\leq Ni$ де $i = 1, ... , k$ та $m = x_1+x_2+ ... +x_k$

або $$\sum_{i=1}^{k}x_i = m$$
...
| 9  | $\|type\_sizes\| = k$         | Кількість елементів у контейнері `type_sizes` повинна дорівнювати кількості типів `type_count` | `validate_type_sizes_count`                     |
| 10 | $\|selected\_counts\| = k$    | Кількість елементів у контейнері selected_counts також повинна дорівнювати `type_count`        | `validate_selected_counts_count`                |
...
$$\text{sum_probability}(N_1,\ldots,N_k,m,j,h) = \sum_{(x_1,\ldots,x_k)\in \text{configurations}} P(x_1,\ldots,x_k)$$
